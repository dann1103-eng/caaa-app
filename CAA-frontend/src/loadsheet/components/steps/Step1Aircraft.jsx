...
export default function Step1Aircraft() {
  ...
  return (
    <div>
        ...
        {AC_KEYS.map(key => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all cursor-pointer ${
              state.currentAC === key
                ? 'bg-[var(--c-brand-700)] text-white border-[var(--c-brand-700)]'
      ...
      {/* Specs bar */}
      <div className="bg-[var(--c-brand-50)] rounded-md p-3 mb-6 grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs">
        <div><span className="text-gray-500 block">Modelo</span><span className="font-semibold text-[var(--c-brand-700)]">{ac.model}</span></div>
        <div><span className="text-gray-500 block">Peso Vacio</span><span className="font-semibold text-[var(--c-brand-700)]">{ac.empty_weight.toLocaleString()} lb</span></div>
        <div><span className="text-gray-500 block">Peso Max</span><span className="font-semibold text-[var(--c-brand-700)]">{ac.max_gross.toLocaleString()} lb</span></div>
        <div><span className="text-gray-500 block">Combustible</span><span className="font-semibold text-[var(--c-brand-700)]">{ac.fuel_cap_gal} gal ({ac.fuel_usable_gal} usable)</span></div>
        <div><span className="text-gray-500 block">Consumo</span><span className="font-semibold text-[var(--c-brand-700)]">{ac.fuel_burn_note}</span></div>
      </div>

      {/* Flight data form */}
      <h2 className="text-sm font-bold text-[var(--c-brand-700)] uppercase tracking-wider mb-4">Datos del Vuelo</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="date" value={state.flightData.date} onChange={e => dispatch({ type: 'SET_FLIGHT_DATA', field: 'date', value: e.target.value })}
            className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:border-[var(--c-brand-700)] focus:ring-1 focus:ring-[var(--c-brand-700)]" />
        </div>
        <div>
