...
- [ ] **Step 2: Vouchera.** En el SELECT de `getReporteVueloInstructor` sumar `v.categoria, v.tipo_instruccion, v.debitar_saldo` y `EXISTS(SELECT 1 FROM movimiento_cuenta mc WHERE mc.id_vuelo = v.id_vuelo AND mc.tipo='CARGO_VUELO') AS se_debito`. En `reporteVueloPdf.js`, para `categoria === 'CHEQUEO_LINEA' && tipo_instruccion === 'REFRESH'` agregar una línea: `Pago: ${se_debito ? 'debitado de saldo' : 'al momento / coordinar con Administración'}`. Ubicarla junto a los metadatos del vuelo (donde el PDF ya rotula tipo/instrucción). ⚠️ El campo debe viajar por la cadena de datos que alimenta el PDF (verificar cómo llega el reporte al modal — `ReporteVueloModal` recibe el objeto del GET; no inventar otra fuente).
